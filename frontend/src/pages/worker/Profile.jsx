...
export default function WorkerProfile() {
  ...
  const [form, setForm] = useState({ nom: '', email: '', telephone: '', ville: '', description: '' });
  ...
  useEffect(() => {
    if (user) setForm({ nom: user.nom || '', email: user.email || '', telephone: user.telephone || '', ville: user.ville || '', description: user.description || '' });
  ...
  const inputStyle = {
    ...
    borderRadius: '10px', border: '1.5px solid #e2e8f0', fontSize: '0.875rem',
  ...
  return (
    <div style={{ fontFamily: 'Inter, sans-serif', backgroundColor: '#ffffff', minHeight: '100vh', padding: '2rem', overflowY: 'auto' }}>
      <div style={{ maxWidth: '560px', margin: '0 auto' }}>
        ...
        <p style={{ color: '#64748b', margin: '0 0 2rem 0' }}>Gérez vos informations professionnelles.</p>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', marginBottom: '2rem', padding: '1.5rem', backgroundColor: '#f8fafc', borderRadius: '14px', border: '1.5px solid #f1f5f9' }}>
          <div style={{ width: 72, height: 72, borderRadius: '50%', backgroundColor: '#2F3E34', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, fontSize: '1.75rem' }}>
            {(user?.nom || 'W')[0].toUpperCase()}
          </div>
          <div>
            <h2 style={{ fontWeight: 700, color: '#1e293b', margin: '0 0 0.25rem 0', fontSize: '1.1rem' }}>{user?.nom}</h2>
            <span style={{ padding: '3px 10px', borderRadius: '999px', backgroundColor: 'rgba(47,62,52,0.1)', color: '#2F3E34', fontSize: '0.75rem', fontWeight: 700 }}>Professionnel</span>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', marginBottom: '3rem' }}>
          ...
          <div>
            <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 600, color: '#374151', marginBottom: '0.4rem' }}>Description / Bio</label>
            <textarea placeholder="Décrivez vos compétences, votre expérience, vos certifications..." rows={4}
              style={{ ...inputStyle, paddingLeft: '1rem', resize: 'vertical', lineHeight: 1.6 }}
        ...
        <div style={{ borderTop: '2px solid #f1f5f9', paddingTop: '2rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
            <div style={{ width: 36, height: 36, borderRadius: '10px', backgroundColor: 'rgba(47,62,52,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <span className="material-symbols-outlined" style={{ color: '#2F3E34', fontSize: '1.1rem' }}>lock</span>
            </div>
            <div>
