...
export default function WorkerLayout() {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', fontFamily: 'Inter, sans-serif', backgroundColor: '#ffffff' }}>
      ...
      {/* ══ SIDEBAR (desktop) ══ */}
      <aside className="wl-sidebar" style={{ width: '288px', backgroundColor: '#2F3E34', display: 'flex', flexDirection: 'column', flexShrink: 0, height: '100vh', position: 'sticky', top: 0 }}>
            ...
            return (
              <Link key={path} to={path} style={{
                ...
                {isMsgs && unreadCount > 0 && (
                  <span className="wl-nav-badge" style={{ position: 'absolute', right: '0.75rem', backgroundColor: '#22c55e', color: 'white', borderRadius: '999px', fontSize: '0.7rem', fontWeight: 700, padding: '1px 7px', minWidth: '20px', textAlign: 'center' }}>
                    {unreadCount}
                  </span>
                )}
              </Link>
        ...
        <div style={{ padding: '1rem', borderTop: '1px solid rgba(255,255,255,0.1)' }}>
          <button onClick={handleLogout} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.75rem 1rem', borderRadius: '8px', width: '100%', background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.875rem', fontWeight: 500, color: 'rgba(245,242,236,0.65)', transition: 'all 0.2s', fontFamily: 'Inter, sans-serif' }}
            ...
            <span className="wl-nav-label">Déconnexion</span>
          </button>
        </div>
      </aside>

      {/* ══ MAIN ══ */}
      <div className="wl-main" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>

        {/* Header */}
        <header className="wl-header" style={{ height: '80px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 2rem', backgroundColor: '#ffffff', borderBottom: '1px solid #f1f5f9', flexShrink: 0 }}>
          <div style={{ minWidth: 0 }}>
            <h2 className="wl-header-title" style={{ fontSize: '1.5rem', fontWeight: 700, color: '#1e293b', margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
              Bonjour, {user?.nom || 'Professionnel'} 👷
            </h2>
            <p className="wl-header-sub" style={{ color: '#64748b', fontSize: '0.875rem', margin: 0 }}>Gérez vos missions et votre profil professionnel.</p>
          </div>
        ...
        <div className="wl-outlet" style={{ flex: 1, overflowY: 'auto' }}>
          <Outlet />
        </div>
      </div>
      ...
      <style>{`
        ...
        @media (max-width: 1024px) {
          .wl-sidebar    { width: 72px !important; }
          .wl-brand      { display: none !important; }
          .wl-nav-label  { display: none !important; }
          .wl-nav-badge  { right: 4px !important; }
          .wl-header     { padding: 0 1.25rem !important; }
        }

        /* ══ Mobile (≤ 768px) ══ */
        @media (max-width: 768px) {
          .wl-sidebar        { display: none !important; }
          .wl-bottom-nav     { display: flex !important; }
          .wl-main           { padding-bottom: 64px; }
          .wl-header         { padding: 0 1rem !important; height: 64px !important; }
          .wl-header-sub     { display: none !important; }
          .wl-header-title   { font-size: 1.1rem !important; }
        }

        /* ══ iPhone SE (≤ 375px) ══ */
        @media (max-width: 375px) {
          .wl-header         { padding: 0 0.75rem !important; }
          .wl-header-title   { font-size: 1rem !important; }
        }

        /* Bottom nav — caché par défaut */
        .wl-bottom-nav {
          display: none;
          position: fixed;
